...
module internal MarketGroups15=
    let getMarketGroup id = 
      ...
      | 2052 -> { MarketGroupData.id= 2052; name= "Caldari"; parentMarketGroupId= Some(2044); typeIds= [| 36641; 36642; 48163; 48164; 53157; 53158; 46097; 52552; 36713; 36714; 40587; 40588; 46830; 46831; 36337; 46098; 36347; 45820; 45821 |]; description= "" } |> Some
      ...
      | 364056 -> { MarketGroupData.id= 364056; name= "Standard"; parentMarketGroupId= Some(364055); typeIds= [|  |]; description= "Standard." } |> Some
      | 365899 -> { MarketGroupData.id= 365899; name= "Blaster Ammo Expansion Units"; parentMarketGroupId= Some(365898); typeIds= [|  |]; description= "Blaster Ammo Expansion Units." } |> Some
      | 365996 -> { MarketGroupData.id= 365996; name= "Federal Marines"; parentMarketGroupId= Some(365995); typeIds= [|  |]; description= "The Gallente store contains items that require loyalty points from the Federal Marines." } |> Some
      | 366190 -> { MarketGroupData.id= 366190; name= "Dropsuit Modules"; parentMarketGroupId= Some(365996); typeIds= [|  |]; description= "" } |> Some
      | 366287 -> { MarketGroupData.id= 366287; name= "Advanced"; parentMarketGroupId= Some(366194); typeIds= [|  |]; description= "" } |> Some
