...
module internal MarketGroups87=
    let getMarketGroup id = 
      ...
      | 2027 -> { MarketGroupData.id= 2027; name= "Minmatar"; parentMarketGroupId= Some(2023); typeIds= [| 36833; 46914; 48518; 47176; 48522; 52432; 52888; 36786; 45977; 40504; 35705 |]; description= "" } |> Some
      ...
      | 2706 -> { MarketGroupData.id= 2706; name= "Jump Filaments"; parentMarketGroupId= Some(2456); typeIds= [| 53984; 53985; 53986; 53987; 54802; 54803; 53977; 53983 |]; description= "Filaments that jump fleets unpredictably to other locations in known space." } |> Some
      ...
      | 353652 -> { MarketGroupData.id= 353652; name= "Light Attack Vehicles"; parentMarketGroupId= Some(353566); typeIds= [|  |]; description= "" } |> Some
      | 354331 -> { MarketGroupData.id= 354331; name= "Standard"; parentMarketGroupId= Some(353569); typeIds= [|  |]; description= "" } |> Some
      | 363449 -> { MarketGroupData.id= 363449; name= "Heavy"; parentMarketGroupId= Some(354471); typeIds= [|  |]; description= " " } |> Some
      | 365001 -> { MarketGroupData.id= 365001; name= "Vehicle Upgrades"; parentMarketGroupId= Some(365497); typeIds= [|  |]; description= "Skills adapted to increasing vehicle effectiveness on the battlefield." } |> Some
      | 366262 -> { MarketGroupData.id= 366262; name= "Advanced"; parentMarketGroupId= Some(366260); typeIds= [|  |]; description= "" } |> Some
